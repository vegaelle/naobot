...
class LineBuffer(object):
    ...
    def decode(self, line, encoding=0):
        try:
            return line.decode(self.encoding[encoding])
        except UnicodeDecodeError:
            if (encoding+1) < len(self.encoding):
                return self.decode(line, encoding+1)
            else:
                return ''
...
class ServerConnection(Connection):
    ...
    def __init__(self, irclibobj, fix_mode=True):
    ...
    def privmsg(self, target, text):
        ...
        if '\n' in text and self.fix_mode:
            ...
            bytes = ('PRIVMSG %s :%s' % (target, text)).encode('utf-8') +\
                    b'\r\n'
            if len(bytes) > 512 and self.fix_mode:
                diff = len(bytes) - len(text)
                length = 512 - diff
                for line in [text[i:i+length] for i in range(0, len(text), length)]:
