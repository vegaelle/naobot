...
class help(stdPlugin):
    ...
    def on_cmd(self, serv, ev, command, args, helper):
        ...
        if command == '':
            ...
            serv.privmsg(helper['target'], u'Pour obtenir l’aide d’un plugin, '
                         u'tapez %shelp <plugin>' %
                         self.bot.conf['command_prefix'])
            serv.privmsg(helper['target'], u'Liste des plugins actifs : %s' %
                         plugin_list)
            return True
        elif command in self.bot.registered_plugins.keys():
            plugin_help = inspect.getdoc(self.bot.registered_plugins[command])
            serv.privmsg(helper['target'], u'Documentation du plugin %s :' %
                         command)
            serv.privmsg(helper['target'], '\t\t'+plugin_help)
            try:
                command_help = inspect.getdoc(getattr(
                ...
                for line in command_help:
                    serv.privmsg(helper['target'], '\t\t'+line %
                                 {'namespace': self.bot.conf['command_prefix']
                                 + command_namespace})
            except AttributeError:
                serv.privmsg(helper['target'], u'Le plugin ne dispose d’aucune'
                             u' commande.')
            return True
        else:
            serv.privmsg(helper['target'], u'Commande inconnue. Tapez %shelp '
                         u'pour une liste des plugins' %
                         self.bot.conf['command_prefix'])
            return True
