...
class karma(stdPlugin):
    u'''Chaque terme suivi de '++' ou '--' se voit attribuer un karma en
    conséquence. Par exemple, « fromage++ ».
    '''
    ...
    def react_karma_pos(self, k):
        ...
        if k <= 32:
            return u"Waou, je n'ai jamais vu quelque chose d'aussi "\
                   u"monstrueusement génial que %s !"
        if k <= 64:
    ...
    def react_karma_neg(self, k):
        ...
        if k <= 128:
            return u"Chuck Norris a cherché pire que %s. Il est revenu en "\
                   u"pleurant."
    ...
    def find_all_karma(self, message, plus):
        ...
        # couples. Quick fix.
        return [x[0] for x in r.findall(message) if len(x[0]) >=
                self.min_word_size]
    ...
    def match_words(self, serv, chan, message):
        ...
        chosen_words = [(w, k) for (w, k) in candidates if abs(k) ==
                        abs(karma)]
